...
BCLocalTcpSocket::BCLocalTcpSocket(const QString &ip, int port)
    :QObject(NULL)
{
    m_nType = -1;
    m_pBoardCard = NULL;
...
BCLocalTcpSocket::~BCLocalTcpSocket()
{
    m_pTcpSocket->disconnectFromHost();
    m_pTcpSocket->deleteLater();

    DestroyBoardCardData(); // 清空板卡数据
}

void BCLocalTcpSocket::RequestConfig(int type)
{
    if (0 != m_nRequestcount) {
        // 写日志
        BCLocalServer *pServer = BCLocalServer::Application();
        pServer->AddLog( QString("TCP REQUEST ERR TIMES: %1. \n RECV: %2 BITS. \n DATA:%3").arg(m_nRequestcount).arg(m_byteRecv.count()).arg(QString(m_byteRecv)) );
    }
    ...
    if (10 == m_nType) {
        m_pTcpSocket->write(QString("getformat\r\n").toLatin1());
        m_pTcpSocket->flush();
    } else {
...
void BCLocalTcpSocket::onRecvTcpMessage()
{
    QByteArray byteRecv = m_pTcpSocket->readAll();
    m_byteRecv.append( byteRecv );
}

void BCLocalTcpSocket::onTimeOut()
{
    m_nTimerTimes++;

    qDebug() << m_byteRecv.length() << "~~~";
    if (10 != m_nType) { // 4000
        // 判断起始符和结束符
        if (m_byteRecv.length() == 12018) { // 4U
            if ((QChar(m_byteRecv.at(0)) == QChar(0xA6))
                ...
                for (int i = 0; i < nGroupCount; i++) {
                    ...
                    // 放基本信息，供下面默认窗口时使用
                    lstGroupDisplayBaseInfo << groupid << posx << posy;
                ...
                for (int i = 0; i < nChannelCount; i++) {
                    ...
                    if ( BCCommon::g_bDefaultWindowOffset ) {
                        int nBeginGID = ssigWindow.gid*3;
                        if (lstGroupDisplayBaseInfo.count() > nBeginGID+2) {
                            x0 -= lstGroupDisplayBaseInfo.at(nBeginGID+1);
                            x1 -= lstGroupDisplayBaseInfo.at(nBeginGID+1);
                            y0 -= lstGroupDisplayBaseInfo.at(nBeginGID+2);
                            y1 -= lstGroupDisplayBaseInfo.at(nBeginGID+2);
                        }
                    }
...
void BCLocalTcpSocket::sortSignalWindow(QList<sSignalWindow> *pList)
{
    qSort(pList->begin(), pList->end(), compareSignalWindow);
}

bool BCLocalTcpSocket::compareSignalWindow(const sSignalWindow &win1, const sSignalWindow &win2)
{
...
void BCLocalTcpSocket::DestroyBoardCardData()
{
