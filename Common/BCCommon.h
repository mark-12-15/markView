...
public:
    ...
    static bool g_bEchoModel;                       // 是否有预监回显模块
    ...
    static int g_nPreviewDirectDevice;              // 预监回显是否直连设备
    static QString g_qsPreviewIP;                   // 预监IP
    static int g_nPreviewPort;                      // 预监port
    static int g_nPreviewTick;                      // 预监tick
    ...
    //加载语言
    //参数为.qm文件路径
    //SetLanguage("../language/ENGLISH.qm");
    static void SetLanguage(const QString &str)
    {
    ...
    static void InitLanguage()
    {
        ...
        if ( !docElem.isNull() ) {
            int nLastIndex = docElem.attribute("LanguageLastIndex").toInt();
            if (nLastIndex < 0 || nLastIndex > 2)
                return;

            QStringList lstQmLanguage;
            lstQmLanguage << QString("../resource/language/SIMPLIFIED_CHINESE.qm")
                            //<< QString("../resource/language/TRADITIONAL_CHINESE.qm")//默认语言问题解决
            ...
            SetLanguage( lstQmLanguage.at(nLastIndex) );
        }
    }
